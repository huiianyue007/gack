    ...
    <div>
        ...
        <div v-loading="loadingTab">
            <el-table :data="tableData" border style="width: 100%" ref="multipleTable" @select-all='selectBox' @select='selectBox'>
                ...
                <el-table-column prop="auditOpinion" label="审核未通过原因" align="center">
                    <template slot-scope="scope">
                        <el-popover trigger="hover" placement="top">
                            <p>拒绝原因种类: {{ scope.row.auditOpinion }}</p>
                            <p>拒绝详情: {{ scope.row.reasoncontent }}</p>
                            <div slot="reference" class="name-wrapper">
                                <el-tag>{{ scope.row.auditOpinion }}</el-tag>
                            </div>
                        </el-popover>
                    </template>
                </el-table-column>
            ...
            <div class="other">
                ...
                <p>审核未通过的服务商品项目: {{auditFailedNum}} </p>
            </div>
            <div class="page">
            ...
            this.$htAjax.post(`${this.$config.back}/guoanmaker/provide/commodity/findCommdity`, {}, {
                ...
                if (data.status === 200) {
                    ...
                    if (data.data.commoditys.length > 0) {//如果有数据的话，组装数据
                        ...
                        that.auditFailedNum = data.data.total;
                    } else {
                        that.tableData = [];
                        that.totalCount = 0;
                        that.auditFailedNum = 0;
                    }
                    that.loadingTab = false;
                }
            }).catch(function(error) {
