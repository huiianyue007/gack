    ...
    <card class = 'view'>
      ...
      <el-form class = 'form' :data = 'specialForm' :model = 'specialForm' :rules = 'specialRules' ref="specialForm" label-width="120px"  label-position="left">
        <el-form-item label = '单位名称' prop = 'invoiceName'>
          <el-input v-model = 'specialForm.invoiceName' :placeholder = 'invoiceList ? invoiceList.invoiceName : "请输入单位名称"'></el-input>
        </el-form-item>
        <el-form-item label = '纳税人识别号' prop = 'taxNumber'>
          <el-input v-model = 'specialForm.taxNumber' :placeholder = 'invoiceList ? invoiceList.taxNumber : "请输入纳税人识别号"'></el-input>
        </el-form-item>
        <el-form-item label = '注册电话' prop = 'telephone' v-if = 'invoiceType === "1"'>
          <el-input v-model = 'specialForm.telephone' :maxlength="11" :placeholder = 'invoiceList ? invoiceList.telephone : "请输入注册电话"'></el-input>
        </el-form-item>
        <el-form-item label = '注册地址' prop = 'address' v-if = 'invoiceType === "1"'>
          <el-input v-model = 'specialForm.address' :placeholder = 'invoiceList ? invoiceList.address : "请输入注册地址"'></el-input>
        </el-form-item>
        <el-form-item label = '开户银行' prop = 'bankName' v-if = 'invoiceType === "1"'>
          <el-input v-model = 'specialForm.bankName' :placeholder = 'invoiceList ? invoiceList.bankName : "请输入开户银行"'></el-input>
        </el-form-item>
        <el-form-item label = '银行账户' prop = 'bankNumber' v-if = 'invoiceType === "1"'>
          <el-input v-model.number = 'specialForm.bankNumber' :placeholder = 'invoiceList ? invoiceList.bankNumber : "请输入银行账户"'></el-input>
        </el-form-item>
        <el-form-item label-width = '0px'>
          <el-checkbox-group v-model="specialForm.isdefault">
    ...
    <div class="view" v-if = 'invoiceType === "1"'>
      <el-form>
        ...
        <el-form-item label = '营业副本'>
          <el-upload class="avatar-uploader" name="companyLogo" action="https://apitest.gack.citic:8082/putImg" :show-file-list="false" :multiple="false" :before-upload="beforeAvatarUploadCopy" :on-success="handleAvatarSuccessCopy">
   ...
   data () {
     ...
     const checkTaxNumber = (rule, value, callback) => {
       ...
       if (value.length < 10 || value.length > 20) {
         return callback(new Error('输入识别号不能大于20位,不能小宇10位'))
       }
     ...
     return {
       ...
       specialRules: {
         ...
         taxNumber: [{ validator: checkTaxNumber, trigger: 'blur' }],
         telephone: [{ required: true, message: '请输入手机号', trigger: 'blur' },
           { pattern: /^1[34578]\d{9}$/, message: '请输入正确的手机号码', trigger: 'blur' }],
         address: [{ required: true, message: '请输入地址', trigger: 'blur' }],
         bankName: [{ required: true, message: '请输入开户银行名称', trigger: 'blur' }],
         bankNumber: [{ validator: checkNumber, trigger: 'blur' }]
       }
     }
   },
   computed: {
         ...
         if (valid) {
           ...
           if (this.invoiceType !== "1") {
             ...
             data = Object.assign(this.specialForm, {
               businessLicenseFile: this.businessFile.replace(/https/ig, 'http'),
           ...
           let url = this.$route.query.id ? 'https://apitest.gack.citic:8081/guoanmaker/personal/invoice/updateInvoiceForPC' :'https://apitest.gack.citic:8081/guoanmaker/personal/invoice/saveInvoiceForPC'
           if (this.$route.query.id) {
