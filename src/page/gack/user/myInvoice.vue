<template>
  <div class="invoice">
    <card>
      <div class="title" slot = 'header'><span class="tip inline-block"></span>我的发票 <el-button type = 'primary' class = 'fr' @click = '$router.push({path: "/user/addinvoice", query: {title: "我的发票"}})'>添加发票信息</el-button></div>
      <card v-for = '(item, key) in invoiceList' :key = 'key' class = 'view' v-if = 'invoiceList.length'>
        ...
        <el-row>
          <el-col :span="8" @click.native = 'setDefault(item)'>
            <img src="~assets/gack/selected.png" alt="" width = '16' height = '16' v-if = 'item.isdefault === "1"'>
            <img src="~assets/gack/unselected.png" alt=""  width = '16' height = '16' v-if = 'item.isdefault === "0"'>
            {{ item.isdefault === "1" ? '默认发票' : '设为默认'}}
          </el-col>
          <el-col :span="8" class = 'text-center' @click.native = '$router.push({path: "/user/addinvoice", query: {id: item.id, title: "我的发票"}})'>
            编辑
          </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      setDefault (opt) {
        this.$htAjax.post('https://apitest.gack.citic:8081/guoanmaker/personal/invoice/updateInvoice', {}, {
          params: {
            id: opt.id,
            isdefault: '1'
          }
      ...
      deleteInvoice (opt) {
        this.$htAjax.post('https://apitest.gack.citic:8081/guoanmaker/personal/invoice/deleteInvoice', {}, {
          params: {
            id: opt.id
          }
        }).then(({ data }) => {
