...
export default {
  beforeRouteEnter (to, from, next) {
    if (from.fullPath == '/') {
      store.commit('setParentPage', '/')
    } else {
      store.commit('setParentPage', 'from.fullPath')
    }
    next()
  },
  mounted () {
    if (document.getElementById('loading_shadow')) {
      document.body.removeChild(window.loadding)
    }
  },
  computed: {
    user () {
  ...
  methods: {
    resetVerCode () {
      ...
      return new Promise((resolve, reject) => {
        this.$htAjax.post(`${this.$config.gack}/guoanmaker/personal/user/getPicCode`).then(res => {
          resolve(res)
        }).catch(error => {
          reject(error)
        })
